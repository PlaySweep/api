class SendSlateNotificationJob < BudweiserJob
  ...
  def perform user_id, slate_id
    ...
    if user.cards.size > 1
      ...
      FacebookMessaging::Standard.deliver(user, "Congratulations #{user.first_name}, you completed your first Bud Light Sweep Contest!\n\nWe’ll notify you of your results when the games are complete - and when you start getting thirsty, here's a $10 Drizly credit that you can redeem right now 🍺!", "NO_PUSH")
      FacebookMessaging::Carousel.deliver_drizly(user)
    end
  end
end