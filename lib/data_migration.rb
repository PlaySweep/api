class DataMigration
  ...
  def create_slate tenant:, team_name:, abbreviation:, owner_id:, start_time:
    Apartment::Tenant.switch(tenant) do
      slate = Slate.create(name: team_name, local: true, owner_id: owner_id, start_time: start_time)
  ...
  def self.update_address address:
    street_number = ""
    route = ""
    address.metadata.select do |meta|
