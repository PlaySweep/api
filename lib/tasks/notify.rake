    ...
    users.each_with_index do |user, index|
      ...
      begin
        if !user.confirmed || user.current_team.nil?
          content = "Week 4 Kickoff is less than 24 hours away! Type play now for more contests and prizes."
          FacebookMessaging::Standard.deliver(user, content, "SILENT_PUSH")
        elsif user.confirmed && user.current_team.present?
          content = "Week 4 Kickoff starts tomorrow! Time to get your answers in and win some prizes!"
          FacebookMessaging::Standard.deliver(user, content)
          FacebookMessaging::Carousel.deliver_team(user)
        elsif user.confirmed
          content = "Week 4 Kickoff starts tomorrow! Type play now for more contests and prizes."
          FacebookMessaging::Standard.deliver(user, content)
        end
