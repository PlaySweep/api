...
def fetch_user_acquisition_data
  teams = Team.active
  CSV.open("#{Rails.root}/tmp/#{(DateTime.current - 2).to_date}_acquisition_data.csv", "wb") do |csv|
    csv << ["Date", "Team", "Attempted Sign Ups", "Confirmed Users", "Facebook", "Instagram Post", "Instagram Story", "Twitter", "Organic"]
    teams.each do |team|
      team_query = User.where('users.created_at BETWEEN ? AND ?', DateTime.current.beginning_of_day - 2, DateTime.current.end_of_day - 2).joins(:roles).where("roles.resource_id = ?", team.id)
      confirmed_team_query = User.where(confirmed: true).where('users.created_at BETWEEN ? AND ?', DateTime.current.beginning_of_day - 2, DateTime.current.end_of_day - 2).joins(:roles).where("roles.resource_id = ?", team.id)
      csv << [(DateTime.current - 2).to_date.strftime("%Y%m%d"), team.name, team_query.count, confirmed_team_query.count, for_referral(resource: team, source: "facebook").count, for_referral(resource: team, source: "instagrampost").count, for_referral(resource: team, source: "instagramstory").count, for_referral(resource: team, source: "twitter").count, for_referral(resource: team, source: "landing_page").count]
    end
  end
end

def for_referral resource:, source:
  User.where('users.created_at BETWEEN ? AND ?', DateTime.current.beginning_of_day - 2, DateTime.current.end_of_day - 2).joins(:roles).where("roles.resource_id = ?", resource.id).where("users.data->>'referral' = :referral", referral: source)
end

def fetch_engagement_data
  slate_ids = Team.all.map do |team|
    Slate.where('start_time BETWEEN ? AND ?', DateTime.current.beginning_of_day - 2, DateTime.current.end_of_day - 2).filtered(team.id).joins(:cards).group(:id).order(Arel.sql("COUNT(cards.id) DESC")).count.keys
  end.flatten
  CSV.open("#{Rails.root}/tmp/#{(DateTime.current - 2).to_date}_engagement_data.csv", "wb") do |csv|
    ...
    slate_ids.each do |slate_id|
      ...
      csv << [(DateTime.current - 2).to_date.strftime("%Y%m%d"), slate_id, slate.team.name, slate.local ? "Local" : "Vs", slate.cards.count, slate.prizes.first.product.category, slate.start_time.strftime("%A").capitalize, slate.cards.map(&:status).reject { |status| status == 'loss' }.count]
    end
  end
end
